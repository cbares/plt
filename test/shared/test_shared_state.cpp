...
BOOST_AUTO_TEST_CASE(State_constructor){
    	shared_ptr<State> state = make_shared<State>(42);
...
BOOST_AUTO_TEST_CASE(State_serialization){
	shared_ptr<State> state1 = make_shared<State>(42);
	shared_ptr<State> state2 = make_shared<State>(10); // Different from state1

	Json::Value valueState1 = state1->serialize();
	state2->unserialize(valueState1);

	BOOST_TEST(state1->players == state2->players);
	BOOST_TEST(state1->rivers == state2->rivers);
